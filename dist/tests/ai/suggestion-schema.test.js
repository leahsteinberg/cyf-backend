    ...
    describe('valid suggestions', () => {
        ...
        it('accepts suggestion without optional metadata', () => {
            const suggestion = {
                ...
                time: { kind: 'NOW' },
                target: { kind: 'FRIEND', friendId: 'abc' },
            };
    ...
    describe('time validation', () => {
        ...
        it('rejects invalid datetime format', () => {
            const suggestion = {
                confidence: 0.5,
                reason: 'Test',
                time: { kind: 'FUTURE', startsAt: 'not-a-date' },
                target: { kind: 'FRIEND', friendId: 'abc' },
            };
            const result = AISuggestedMeetingSchema.safeParse(suggestion);
            expect(result.success).toBe(false);
...
describe('AISuggestionResponseSchema', () => {
            ...
            suggestions: [
                {
                    ...
                    time: { kind: 'NOW' },
                    target: { kind: 'FRIEND', friendId: 'user-1' },
                },
                {
                    confidence: 0.7,
                    reason: 'Second suggestion',
                    time: { kind: 'FUTURE', startsAt: '2025-03-18T15:00:00Z' },
                    target: { kind: 'FRIEND', friendId: 'user-2' },
                },
            ],
    ...
    it('rejects more than 3 suggestions', () => {
        const response = {
            suggestions: [
                { confidence: 0.8, reason: 'One', time: { kind: 'NOW' }, target: { kind: 'FRIEND', friendId: '1' } },
                { confidence: 0.7, reason: 'Two', time: { kind: 'NOW' }, target: { kind: 'FRIEND', friendId: '2' } },
                { confidence: 0.6, reason: 'Three', time: { kind: 'NOW' }, target: { kind: 'FRIEND', friendId: '3' } },
                { confidence: 0.5, reason: 'Four', time: { kind: 'NOW' }, target: { kind: 'FRIEND', friendId: '4' } },
            ],
        };
        const result = AISuggestionResponseSchema.safeParse(response);
        expect(result.success).toBe(false);
    ...
    it('accepts response without optional reasoning', () => {
        const response = {
            suggestions: [
                { confidence: 0.8, reason: 'Test', time: { kind: 'NOW' }, target: { kind: 'FRIEND', friendId: '1' } },
            ],
        };
        const result = AISuggestionResponseSchema.safeParse(response);
        expect(result.success).toBe(true);
...
//# sourceMappingURL=suggestion-schema.test.js.map