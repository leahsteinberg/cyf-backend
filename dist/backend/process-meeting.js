...
import { getEffectiveTimeType, getEffectiveTargetType, IMMEDIATE_TIME_TYPE, UNKNOWN_TIME_TYPE, DISMISSED_DRAFT_MEETING_STATE, ACCEPTED_MEETING_STATE, PAST_MEETING_STATE, REJECTED_MEETING_STATE, ACCEPTED_OFFER_STATE, REJECTED_OFFER_STATE, EXPIRED_OFFER_STATE } from '../types.js';
 ...
 */
export const processOffersForNewMeeting = async (meeting) => {
 ...
 */
async function createParallelOffers(meeting) {
    ...
    const offerPromises = recipientIds.map(friendId => makeOffer({ meeting, userOfferedId: friendId, expiresAt }));
 ...
 */
async function processParallelOffers(meeting) {
    const meetingId = meeting.id;
    ...
    const allRejectedOrExpired = offers.every(o => o.offerState === REJECTED_OFFER_STATE || o.offerState === EXPIRED_OFFER_STATE);
    if (allRejectedOrExpired && offers.length > 0) {
        await setMeetingState({ meetingId, meetingState: REJECTED_MEETING_STATE });
    }
    return meeting;
}
//# sourceMappingURL=process-meeting.js.map