...
import { AISuggestionResponseSchema } from './suggestion-schema.js';
...
export async function generateSuggestions(userId) {
    ...
    try {
        ...
        const validSuggestions = validated.data.suggestions.filter(s => validateSuggestion(s, context));
...
export async function createMeetingFromSuggestion(userId, suggestion) {
    const now = new Date();
