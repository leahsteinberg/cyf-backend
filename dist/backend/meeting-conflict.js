import { isBroadcastMeeting, PAST_MEETING_STATE, DRAFT_MEETING_STATE, DISMISSED_DRAFT_MEETING_STATE, UNKNOWN_TIME_TYPE, getEffectiveTimeType } from '../types.js';
/**
 ...
 */
export const findMeetingTimeConflict = ({ userCreatedMeetings, userAcceptedMeetings, scheduledFor, scheduledEnd, excludeMeetingId }) => {
    const newMeetingStart = new Date(scheduledFor);
    const newMeetingEnd = new Date(scheduledEnd);
    // Helper to check if two time ranges overlap
    const hasTimeOverlap = (existingStart, existingEnd) => {
    ...
    const shouldExcludeFromConflicts = (meeting) => {
...
//# sourceMappingURL=meeting-conflict.js.map