...
export const getAcceptedMeetings = async ({ acceptedUserId }) => {
    const meetings = await prisma.meeting.findMany({
        where: {
            acceptedUserIds: {
                has: acceptedUserId
...
export const getUserFromMeetingId = async (meetingId) => {
    const meeting = await prisma.meeting.findUnique({
        where: { id: meetingId },
        include: {
            userFrom: true,
 ...
 */
export const enrichMeetingsWithAcceptedUsers = async (meetings) => {
    // Collect all unique acceptedUserIds across all meetings
    const allAcceptedUserIds = new Set();
...
//# sourceMappingURL=meeting-lookup.js.map