...
import { c as createAuthMiddleware, g as getSessionFromCtx, a as createAuthEndpoint, B as BASE_ERROR_CODES } from './better-auth.ejRQe6H4.mjs';
    ...
    endpoints: {
      ...
      setRole: createAuthEndpoint(
        "/admin/set-role",
        {
          ...
          body: z.object({
            ...
            role: z.union([
              z.string().describe("The role to set. `admin` or `user` by default"),
              z.array(
                z.string().describe("The roles to set. `admin` or `user` by default")
              )
            ]).describe(
      ...
      adminUpdateUser: createAuthEndpoint(
        "/admin/update-user",
        {
          ...
          body: z.object({
            userId: z.coerce.string().describe("The user id"),
            data: z.record(z.any(), z.any()).describe("The user data to update")
          }),
          use: [adminMiddleware],
          metadata: {
            openapi: {
      ...
      listUsers: createAuthEndpoint(
        "/admin/list-users",
        {
          ...
          use: [adminMiddleware],
          query: z.object({
            searchValue: z.string().optional().describe('The value to search for. Eg: "some name"'),
            searchField: z.enum(["email", "name"]).describe(
              'The field to search in, defaults to email. Can be `email` or `name`. Eg: "name"'
            ).optional(),
            searchOperator: z.enum(["contains", "starts_with", "ends_with"]).describe(
      ...
      banUser: createAuthEndpoint(
        "/admin/ban-user",
        {
          ...
          body: z.object({
             ...
             */
            banExpiresIn: z.number().describe("The number of seconds until the ban expires").optional()
          }),
          use: [adminMiddleware],
          metadata: {
            openapi: {
      ...
      userHasPermission: createAuthEndpoint(
        "/admin/has-permission",
        {
          method: "POST",
          body: z.object({
            userId: z.coerce.string().optional().describe(`The user id. Eg: "user-id"`),
            role: z.string().optional().describe(`The role to check permission for. Eg: "admin"`)
          }).and(
            z.union([
              z.object({
