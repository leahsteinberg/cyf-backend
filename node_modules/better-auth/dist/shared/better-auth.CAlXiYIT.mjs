...
import { a as createAuthEndpoint, g as getSessionFromCtx, s as sessionMiddleware, c as createAuthMiddleware } from './better-auth.ejRQe6H4.mjs';
...
import './better-auth.CNeuX76b.mjs';
  ...
  return createAuthEndpoint(
    "/api-key/create",
    {
      ...
      body: z.object({
        ...
        expiresIn: z.number().describe("Expiration time of the Api Key in seconds").min(1).optional().nullable().default(null),
        userId: z.coerce.string().describe(
          "User Id of the user that the Api Key belongs to. server-only"
        ).optional(),
        prefix: z.string().describe("Prefix of the Api Key").regex(/^[a-zA-Z0-9_-]+$/, {
          message: "Invalid prefix format, must be alphanumeric and contain only underscores and hyphens."
        }).optional(),
        remaining: z.number().describe("Remaining number of requests. Server side only").min(0).optional().nullable().default(null),
        metadata: z.any().optional(),
        refillAmount: z.number().describe(
          "Amount to refill the remaining count of the Api Key. server-only"
        ).min(1).optional(),
        refillInterval: z.number().describe(
          "Interval to refill the Api Key in milliseconds. server-only"
        ).optional(),
        rateLimitTimeWindow: z.number().describe(
        ...
        rateLimitMax: z.number().describe(
        ...
        rateLimitEnabled: z.boolean().describe("Whether the key has rate limiting enabled. server-only").optional(),
        permissions: z.record(z.string(), z.array(z.string())).describe("Permissions of the Api Key.").optional()
      }),
      metadata: {
        openapi: {
  ...
  return createAuthEndpoint(
    "/api-key/update",
    {
      ...
      body: z.object({
        ...
        enabled: z.boolean().describe("Whether the Api Key is enabled or not").optional(),
        remaining: z.number().describe("The number of remaining requests").min(1).optional(),
        refillAmount: z.number().describe("The refill amount").optional(),
        refillInterval: z.number().describe("The refill interval").optional(),
        metadata: z.any().optional(),
        expiresIn: z.number().describe("Expiration time of the Api Key in seconds").min(1).optional().nullable(),
        rateLimitEnabled: z.boolean().describe("Whether the key has rate limiting enabled.").optional(),
        rateLimitTimeWindow: z.number().describe(
        ...
        permissions: z.record(z.string(), z.array(z.string())).describe("Update the permissions on the API Key. server-only.").optional().nullable()
      }),
      metadata: {
        openapi: {
  ...
  return createAuthEndpoint(
    "/api-key/verify",
    {
      ...
      body: z.object({
        ...
        permissions: z.record(z.string(), z.array(z.string())).describe("The permissions to verify.").optional()
      }),
      metadata: {
        SERVER_ONLY: true
      }
