...
import { getFriendshipsUser1Side, getFriendshipsUser2Side, getFriendsWithDetails } from './query/friendship-lookup.js';

export const getFriends = async (id: string): Promise<User[]> => {
    const friends = await getFriendsWithDetails({ userId: id });
    return friends;
}
