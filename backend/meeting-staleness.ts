 ...
 */
export const isMeetingStale = async (meeting: Meeting): Promise<boolean> => {
 ...
 */
export const shouldShowMeeting = async (meeting: Meeting): Promise<boolean> => {
 ...
 */
export const transitionStaleMeetingToPast = async (meeting: Meeting): Promise<void> => {
  const stale = await isMeetingStale(meeting);
 ...
 */
export const filterAndCleanMeetings = async (
  meetings: Meeting[],
  transitionStale: boolean = false
): Promise<Meeting[]> => {
 ...
 */
export const isMeetingStaleSynchronous = (meeting: Meeting): boolean => {
  if (TERMINAL_STATES.includes(meeting.meetingState)) {
    return false;
  }
 ...
 */
export const filterMeetingsSynchronous = (meetings: Meeting[]): Meeting[] => {
